...
export default function Home() {
  ...
  const wheelX = useTransform(springX, [-500, 500], [-10, 10]);
  const wheelY = useTransform(springY, [-500, 500], [-10, 10]);
  ...
  return (
    <main className="relative w-screen h-[100dvh] overflow-hidden text-white font-sans">
      ...
      {/* 主体内容：转盘 - 居中偏上布局 */}
      <motion.div
        style={{ x: wheelX, y: wheelY }}
        className="absolute inset-0 flex items-center justify-center z-10 pb-[10vh] pointer-events-none"
      >
        <div className="scale-90 md:scale-100 transition-transform pointer-events-auto">
           <Wheel />
        </div>
      </motion.div>
